...
# ================= LOAD MODEL =================
rf_model = joblib.load("aadhaar_rf_model.pkl")
label_encoder = joblib.load("label_encoder1.pkl")

# ================= WINDOW =================
root = tk.Tk()
root.title("Fake / Real Aadhaar Card Detection (RF)")
root.state("zoomed")
...
tk.Label(
    header,
    text="🛡️ FAKE / REAL AADHAAR CARD DETECTION SYSTEM (RANDOM FOREST)",
...
def upload_image():
    ...
    if fn:
        ...
        result_label.config(text="IMAGE LOADED", fg="#FFEB3B", bg="#000000")

def preprocess():
    if not fn:
        result_label.config(text="NO IMAGE SELECTED", fg="red", bg="#000000")
        return
    ...
    result_label.config(text="PREPROCESSING DONE", fg="#00E5FF", bg="#000000")

def detect():
    if not fn:
        result_label.config(text="NO IMAGE", fg="red", bg="#000000")
        return
...
side_btn("RF DETECTION", detect, "#6A1B9A").pack(pady=15)
side_btn("EXIT", root.destroy, "#C62828").pack(side="bottom", pady=15)

# ================= FOOTER =================
footer = tk.Label(
    root,
    text="Machine Learning Based Fake/Real Aadhaar Detection | Random Forest Model",
    bg="#000814",
    fg="#90CAF9",
    font=("Segoe UI", 10),
    height=2
